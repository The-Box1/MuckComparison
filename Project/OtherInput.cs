...
public class OtherInput : MonoBehaviour
{
	...
	{
		return craftingState switch
		{
			CraftingState.Workbench => ItemManager.Instance.GetItemByName("Workbench").id, 
			CraftingState.Furnace => ItemManager.Instance.GetItemByName("Furnace").id, 
			CraftingState.Anvil => ItemManager.Instance.GetItemByName("Anvil").id, 
			CraftingState.Fletch => ItemManager.Instance.GetItemByName("Fletching Table").id, 
			CraftingState.Cauldron => ItemManager.Instance.GetItemByName("Cauldron").id, 
			_ => -1, 
		};
	}
